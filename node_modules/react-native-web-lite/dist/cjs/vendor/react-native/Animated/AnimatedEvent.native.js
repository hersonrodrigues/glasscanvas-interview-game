...
var import_react_native_web_internals = require("react-native-web-internals"), import_NativeAnimatedHelper = __toESM(require("./NativeAnimatedHelper")), import_NativeAnimatedHelper2 = require("./NativeAnimatedHelper"), import_AnimatedValue = __toESM(require("./nodes/AnimatedValue"));
...
function attachNativeEvent(viewRef, eventName, argMapping) {
  var eventMappings = [], traverse = function(value, path) {
    if (_instanceof(value, import_AnimatedValue.default))
...
function validateMapping(argMapping, args) {
  var validate = function(recMapping, recEvt, key) {
    if (_instanceof(recMapping, import_AnimatedValue.default)) {
      (0, import_react_native_web_internals.invariant)(typeof recEvt == "number", "Bad mapping of event key " + key + ", should be number but got " + (typeof recEvt > "u" ? "undefined" : _type_of(recEvt)));
      return;
    }
    if (typeof recEvt == "number") {
      (0, import_react_native_web_internals.invariant)(_instanceof(recMapping, import_AnimatedValue.default), "Bad mapping of type " + (typeof recMapping > "u" ? "undefined" : _type_of(recMapping)) + " for key " + key + ", event value must map to AnimatedValue");
      return;
    }
    (0, import_react_native_web_internals.invariant)(typeof recMapping == "object", "Bad mapping of type " + (typeof recMapping > "u" ? "undefined" : _type_of(recMapping)) + " for key " + key), (0, import_react_native_web_internals.invariant)(typeof recEvt == "object", "Bad event of type " + (typeof recEvt > "u" ? "undefined" : _type_of(recEvt)) + " for key " + key);
